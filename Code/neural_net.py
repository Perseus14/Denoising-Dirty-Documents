...
def model_train():
	...
	#To deal with Memmory error divide by 2
...
def test(model):
    ...
    for fname in os.listdir('../Data/test_real/'):
        test_image = load_image(os.path.join('../Data/test_real', fname))
        test_x = feature_matrix(test_image)

        y_pred, = predict(test_x)
        output = y_pred.reshape(test_image.shape)*255.0

        cv2.imwrite('NN/original_real_' + fname, test_image)
        cv2.imwrite('NN/cleaned_real_' + fname, output)
